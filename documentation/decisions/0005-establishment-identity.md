...
## Context
Establishments get inspected repeatedly and can change names or owners over time; the same physical
place may even appear under different ids. True cross-record identity is an entity-resolution problem.
Usefully, all three sources do carry a native facility/permit id (NY `nys_health_operation_id`, LA
`FACILITY ID`, ABQ permit #).

## Decision
Key each establishment by its source's native facility/permit id via a deterministic
`establishment_uuid` ([0007](0007-deterministic-uuids.md)). No fuzzy matching, no cross-source
linking.
...
- Upgrade path: an entity-resolution stage (address and name normalization/blocking) that assigns a
  stable cross-source `establishment_uuid`, deferred. This ADR is superseded when that lands.
