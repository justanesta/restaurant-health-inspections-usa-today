...
Committing full history to git on every run would bloat the repo and slow CI, and it adds nothing to a
proof of *approach*.
...
Both are one-line changes; `sample_limit: null` plus a larger `window_days` gives a full pull.

## Consequences
- The committed `data/` slice is small, real, and diffable; CI is fast.
- LA extraction still downloads the whole CSV and filters client-side (the source notes point at the
