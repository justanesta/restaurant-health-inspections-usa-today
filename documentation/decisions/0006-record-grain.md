...
violations pre-aggregated into a text blob plus counts; LA = one row per **program-element per
inspection**, with violations in a **separate file** (one inspection to many violation rows); ABQ =
one row per **inspection** in the weekly report. "One inspection" therefore means three different
things.

## Decision
The unified record grain is **one inspection event**. Violations are a **nested array** on the record
rather than separate rows. The establishment is a separate identity carried via `establishment_uuid`
